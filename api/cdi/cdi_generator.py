...
from cdi.config import (  # Import the constants from the config file
    DESCRIPTOR_VARIABLE,
    CONCEPT_SCHEME,
    STRING_TYPE,
    COLUMN_ONE,
...
def create_key_value_data_store_structures(key_values: list[(str, str)], 
                                            column_one_key: str = COLUMN_ONE, 
                                            energy_values: list = DEFAULT_ENERGY_VALUES) -> Graph:
    ...
    svd_dv_g = add_substantive_value_domain_to_graph(DESCRIPTOR_VARIABLE, CONCEPT_SCHEME)

    cs_dv_g = add_concept_scheme_to_graph(DESCRIPTOR_VARIABLE, energy_values)

    vvc_g = add_variable_value_component_to_graph()

    iv_vvc_g = add_instance_variable_to_graph("variableValueComponent")

    svd_vvc_g = add_substantive_value_domain_to_graph("variableValueComponent", STRING_TYPE)

    key_value_g = Graph()

    for key, value in key_values:
        c_dv_g = add_concept_to_graph(DESCRIPTOR_VARIABLE, key)
